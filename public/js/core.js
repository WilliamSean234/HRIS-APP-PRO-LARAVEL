...
function loadPage(pageId) {
    ...
    // 2. Load content - pakai JavaScript
    // fetch(`pages/${pageId}.blade.php`)
    //     .then(response => {
    //         if (!response.ok) {
    //             // Memberikan pesan error yang lebih jelas (seperti yang terlihat pada gambar)
    //             throw new Error(`Failed to fetch pages/${pageId}.blade.php`);
    //         }
    ...
    //     })
    //     .catch(error => {
    //         contentArea.innerHTML = `<div class="card wide-card"><p class="text-danger">Error: ${error.message}. Cek apakah file pages/${pageId}.blade.php ada.</p></div>`;
    //         console.error(error);
    //     });

    // 2. Load content (Menggunakan Route Laravel)
    // Kita panggil route yang kita definisikan, misalnya /content/dashboard
    const routeUrl = `/content/${pageId}`;
    fetch(routeUrl)
        .then(response => {
            if (!response.ok) {
                // Tampilkan pesan error jika route tidak ditemukan (404)
                throw new Error(`Failed to fetch ${routeUrl}. Status: ${response.status}`);
            }
            return response.text();
        })
        .then(html => {
            contentArea.innerHTML = html;
            // Pastikan Anda memanggil fungsi yang benar untuk inisialisasi JS di halaman baru
            initializePage(pageId);
        })
        .catch(error => {
            contentArea.innerHTML = `<div class="card wide-card"><p class="text-danger">Error: ${error.message}. Cek Route atau View Anda.</p></div>`;
            console.error(error);
        });
